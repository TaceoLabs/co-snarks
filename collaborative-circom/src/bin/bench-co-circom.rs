...
/// Generates and verifies a proof for a given circuit using snarkjs and co-circom. Measures prover & verifier time and also measures witness generation time when run with --gen-wtns.
struct Cli {
    /// Input: The path to the proving key (.zkey) file, generated by snarkjs setup phase
    #[arg(long, short)]
    zkey: PathBuf,
