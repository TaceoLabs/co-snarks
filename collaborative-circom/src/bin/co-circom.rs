...
enum Commands {
    ...
    /// Recombine witness sha
    RecombineWitness {
        /// The path to the input share file prefix
        #[arg(long)]
        input: PathBuf,
        /// The path of the witness to check against
        #[arg(long)]
        check_witness: PathBuf,
        /// The path to the r1cs file, generated by Circom compiler
        #[arg(long)]
        r1cs: PathBuf,
        /// The MPC protocol to be used
...
fn main() -> color_eyre::Result<ExitCode> {
    ...
    match args.command {
        ...
        Commands::RecombineWitness {
            input,
            protocol: _,
            check_witness,
            r1cs,
        } => {
            let mut shared_witnesses = Vec::new();
            for i in 0..3 {
                let path = input.with_extension(format!("wtns.{}.shared", i));
                file_utils::check_file_exists(&path)?;
                let file =
                    BufReader::new(File::open(&path).context("while opening witness share file")?);
                ...
                shared_witnesses.push(witness_share);
            }
            file_utils::check_file_exists(&check_witness)?;
            // read the Circom witness file
            let witness_file =
                BufReader::new(File::open(&check_witness).context("while opening witness file")?);
            let witness = Witness::<ark_bn254::Fr>::from_reader(witness_file)
                .context("while parsing witness file")?;
            ...
            for (i, val) in witness.values.iter().enumerate() {
                if i < r1cs.num_inputs {
                    for j in 0..3 {
                        if shared_witnesses[j].public_inputs[i] != *val {
                            tracing::error!(
                                "Public input at index {} does not match original witness",
                                i
                            );
                            break;
                        }
                ...
                let recombined = aby3::utils::combine_field_element(
                    shared_witnesses[0].witness.get(i - r1cs.num_inputs),
                    shared_witnesses[1].witness.get(i - r1cs.num_inputs),
                    shared_witnesses[2].witness.get(i - r1cs.num_inputs),
                );
                if recombined != *val {
                    tracing::error!(
                        "Recombined value at index {} does not match original witness: have {}, want {}",
                        i,
                        recombined,
                        *val
                    );
            ...
            assert_eq!(
                shared_witnesses[0].public_inputs.len() + shared_witnesses[0].witness.len(),
                witness.values.len()
            );
        }
