...
{
    I::from_xy(
        ark_to_icicle_base(&ark_affine.x().unwrap()),
        ark_to_icicle_base(&ark_affine.y().unwrap()),
    )
}
...
where
    ...
    I: Field,
 {
    unsafe { *transmute::<&T, &I>(&ark_scalar) }
}
...
pub trait ArkIcicleBridge {
    ...
    type IcicleScalarField: Field + VecOps<Self::IcicleScalarField> + NTT<Self::IcicleScalarField, Self::IcicleScalarField> + MontgomeryConvertible;
