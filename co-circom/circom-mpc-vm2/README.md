...
`MpcAccelerator` is a registry of *component* and *function* accelerators: driver-specific fast paths that replace a whole template/function body instead of interpreting its instructions — e.g. a Rep3-native Poseidon2 permutation with precomputed randomness. The Rep3 driver pools that S-box precomputation per state size, refilling in geometrically growing chunks (the refill costs three communication rounds however many permutations it covers), so a circuit hashing `N` times pays `O(log N)` precompute round-trips instead of `N`. The predefined set (`sqrt_0`, `Num2Bits`, `AddBits`, `IsZero`, `Poseidon2`) is toggled through `VMConfig::accelerator`; custom ones are added via `WitnessExtension::register_accelerator_component`/`_function` before the first run.
