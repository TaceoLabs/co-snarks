...
use circom_types::groth16::ConstraintMatrix;
use circom_types::traits::CheckElement;
use clap::{Args, Parser, Subcommand, ValueEnum};
use co_circom::{
    Bls12_381, Bn254, CircomArkworksPairingBridge, CircomArkworksPrimeFieldBridge, CircomReduction,
    CoCircomCompiler, CompilerConfig, Compression, Groth16, Groth16JsonVerificationKey,
    Groth16Proof, Groth16ZKey, LibsnarkReduction, Pairing, Plonk, PlonkJsonVerificationKey,
...
/// An enum representing the Groth16 reduction to use.
#[derive(Debug, Clone, ValueEnum, Serialize, Deserialize, Default)]
#[clap(rename_all = "lower")]
pub enum Groth16Reduction {
    /// The circom reduction.
    #[default]
    Circom,
    /// The libsnark reduction.
    Libsnark,
}

impl std::fmt::Display for Groth16Reduction {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Groth16Reduction::Circom => write!(f, "circom"),
            Groth16Reduction::Libsnark => write!(f, "libsnark"),
        }
    }
}
...
impl ValueEnum for Curve {
    fn value_variants<'a>() -> &'a [Self] {
        &[Curve::BN254, Curve::BLS12_381, Curve::BLS12_377]
    }
    ...
    fn to_possible_value(&self) -> Option<clap::builder::PossibleValue> {
        match self {
            ...
            Curve::BLS12_377 => Some(clap::builder::PossibleValue::new("BLS12-377")),
        }
    }
}
...
impl std::fmt::Display for Curve {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Curve::BLS12_377 => write!(f, "BLS12-377"),
        }
    }
}
...
pub struct SplitWitnessCli {
    ...
    /// The number of inputs (if known, else pass r1cs to get number of inputs)
    #[arg(long)]
    #[serde(skip_serializing_if = "::std::option::Option::is_none")]
...
pub struct GenerateProofCli {
    ...
    /// The paths to the 3 Groth16 matrices
    #[arg(long, value_delimiter = ',')]
    #[serde(skip_serializing_if = "::std::option::Option::is_none")]
    pub matrices: Option<Vec<PathBuf>>,
    ...
    #[arg(short, long, default_value_t = 1)]
    pub threshold: usize,
    /// The Groth reduction to be used
    #[arg(long, value_enum)]
    #[serde(skip_serializing_if = "::std::option::Option::is_none")]
    pub groth16_reduction: Option<Groth16Reduction>,
}
    ...
    match args.command {
        Commands::SplitWitness(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_split_witness::<Bls12_377>(config),
            }
        }
        Commands::SplitInput(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_split_input::<Bls12_377>(config),
            }
        }
        Commands::MergeInputShares(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_merge_input_shares::<Bls12_377>(config),
            }
        }
        Commands::GenerateWitness(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_generate_witness::<Bls12_377>(config),
            }
        }
        Commands::TranslateWitness(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_translate_witness::<Bls12_377>(config),
            }
        }
        Commands::GenerateProof(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_generate_proof::<Bls12_377>(config),
            }
        }
        Commands::Verify(cli) => {
            ...
            match config.curve {
                ...
                Curve::BLS12_377 => run_verify::<Bls12_377>(config),
            }
        }
    }
}
    ...
    P::BaseField: CircomArkworksPrimeFieldBridge,
{
    ...
    let num_inputs = if let Some(num_inputs) = config.num_inputs {
        num_inputs
    } else if let Some(path) = config.r1cs {
        // read the circom r1cs file
        let r1cs_file = BufReader::new(File::open(path).context("while opening r1cs file")?);
        let r1cs = R1CS::<P>::from_reader(r1cs_file).context("while parsing r1cs file")?;
        r1cs.num_inputs
    } else {
        eyre::bail!("either r1cs path or num_inputs must be present");
    };
    ...
    match protocol {
        ...
        MPCProtocol::SHAMIR => {
            // create witness shares
            let start = Instant::now();
            let shares = co_circom::split_witness_shamir::<P>(num_inputs, witness, t, n);
            let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
            tracing::info!("Split witness took {duration_ms} ms");
    ...
    P::BaseField: CircomArkworksPrimeFieldBridge,
{
    ...
    let public_input = match proof_system {
        ProofSystem::Groth16 => {
            let zkey = Arc::new(if let Some(matrices) = config.matrices {
                let pk = ProvingKey::<P>::deserialize_uncompressed_unchecked(&zkey_file)?;
                let [a_matrix, b_matrix, c_matrix] = matrices
                    .iter()
                    .map(|path| {
                        ConstraintMatrix::<P::ScalarField>::deserialize_uncompressed(File::open(
                            path,
                        )?)
                    })
                    .collect::<Result<Vec<_>, _>>()?
                    .try_into()
                    .expect("matrices must have len 3 (a, b, c)");
                Groth16ZKey::<P>::from_pk_and_matrices(pk, a_matrix, b_matrix, c_matrix)
            } else {
                Groth16ZKey::<P>::from_reader(&zkey_file, check).context("while parsing zkey")?
            });
            ...
            let (proof, public_input) = match protocol {
                MPCProtocol::REP3 => {
                    ...
                    let start = Instant::now();
                    let (proof, mpc_net) = match config.groth16_reduction.unwrap_or_default() {
                        Groth16Reduction::Circom => {
                            Rep3CoGroth16::prove::<CircomReduction>(mpc_net, zkey, witness_share)?
                        }
                        Groth16Reduction::Libsnark => {
                            Rep3CoGroth16::prove::<LibsnarkReduction>(mpc_net, zkey, witness_share)?
                        }
                    };
                    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
                ...
                MPCProtocol::SHAMIR => {
                    ...
                    let start = Instant::now();
                    let (proof, mpc_net) =
                        ShamirCoGroth16::prove::<CircomReduction>(mpc_net, t, zkey, witness_share)?;
                    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
                    tracing::info!("Generate proof took {duration_ms} ms");
                    // network is shutdown in drop, which can take seom time with quinn
                    drop(mpc_net);
