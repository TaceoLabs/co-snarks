...
│     code removal → jump cleanup → index-constant            │
│     propagation → copy propagation → var-value forwarding → │
│     dead-store elimination → common-subexpression           │
...
- **Loops** (`lower_loop`): three compilation strategies, chosen per loop:
  ...
  *Conformance detection* (`detect_conforming`) accepts simple ascending/descending counters with a constant step, a *loop-invariant* bound (a literal, a scalar var the body never writes, or an invariant expression over such vars — with a statically known value whenever the expression evaluates over tracked constants, `eval_const_expr`), and exactly one write to the counter anywhere in the body (usually last, but e.g. anonymous-component desugaring appends statements after it). The counter's pre-loop value must be statically known: tracked stores are followed through straight-line code *and* through expressions over already-known vars (InsertionSort's inner `j = i - 1` under an unrolled outer loop conforms; an init flowing through a runtime branch join does not). Rejections are traced with their reason at trace level. A loop nested inside a runtime branch is special: the branch condition may be secret-shared, so a data-dependent jump could leak it — such loops are *force-unrolled* regardless of the size threshold, and compilation fails if their trip count isn't statically known.
...
A `ToAddress` node's field-expression operand gets its own symbolic evaluation first (`eval_field_linear`): a linear form over at most one mirrored/unrolled loop variable — e.g. sha256compression's `inp[t*32+31-k]`, which circom's front end flattens into one field expression rather than address arithmetic — folds to `Affine`/`Const` with no runtime conversion, provided the final coefficients are non-negative `u32`s (which is also what makes the fold exact: signed intermediates that cancel are fine, while a form like `t*32 - 33` that is only in range for large enough `t` conservatively keeps the runtime `ToIndex` and its range-error behavior).
