...
def gen_commitments(guesses, addresses, rs):
    ...
    for i in range(len(guesses)):
        ...
        if run_circom(CommitScript, Input, Lib, "key_commitment.out") == False:
            return None
...
def get_winner(guesses, addresses, rs, commitments):
    ...
    if run_circom(WinnerScript, Input, Lib, "key_winner.out") == False:
        return None
...
def simple_test():
    ...
    res = get_winner(inputs, addresses, rs, commitments)
    if res == None:
        print("  Test failed\n")
        return
    (guess, address) = res
