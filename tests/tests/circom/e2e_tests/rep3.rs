...
use co_groth16::{CircomReduction, Groth16};
use co_plonk::Plonk;
...
macro_rules! e2e_test {
    ($name: expr) => {
        add_test_impl!(Groth16, CircomReduction, Bn254, $name);
        add_test_impl!(Groth16, CircomReduction, Bls12_381, $name);
...
macro_rules! add_test_impl {
    ($proof_system: ident, $reduction: expr, $curve: ident, $name: expr) => {
        paste::item! {
            #[test]
            fn [< e2e_proof_ $name _ $curve:lower _ $proof_system:lower>] () {
                ...
                for (net, x, zkey) in izip!(
                    ...
                    threads.push(thread::spawn(move || {
                        [< Rep3Co $proof_system>]::<$curve, PartyTestNetwork>::prove::<[<$reduction>]>(net, zkey, x).unwrap().0
                    }));
                }
                ...
                assert_eq!(der_proof, result2);
                    $proof_system::<$curve>::verify(&vk.into(), &der_proof, &public_input).expect("can verify");
            }
            ...
            fn [< e2e_proof_verify_snarkjs_proof_ $name _ $curve:lower _ $proof_system:lower>] () {
                ...
                $proof_system::<$curve>::verify(&vk.into(), &snarkjs_proof, &public_input.values).expect("can verify");
            }
        }
    };
