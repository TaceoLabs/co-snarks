mod field_share {
    ...
    fn shamir_add_inner(num_parties: usize, threshold: usize) {
        ...
        for (tx, x, y) in izip!(tx, x_shares, y_shares) {
            spawn_pool(move || tx.send(arithmetic::add(x, y)).unwrap());
        }
    ...
    fn shamir_sub_inner(num_parties: usize, threshold: usize) {
        ...
        for (tx, x, y) in izip!(tx, x_shares, y_shares) {
            spawn_pool(move || tx.send(arithmetic::sub(x, y)).unwrap());
        }
    ...
    fn shamir_mul2_then_add_inner(num_parties: usize, threshold: usize) {
        let nets = TestNetwork::party_networks(num_parties);
        let mut rng = thread_rng();
        let x = ark_bn254::Fr::rand(&mut rng);
        let y = ark_bn254::Fr::rand(&mut rng);
        let x_shares = shamir::share_field_element(x, threshold, num_parties, &mut rng);
        ...
        for (net, tx, x, y) in izip!(nets, tx, x_shares, y_shares) {
            spawn_pool(move || {
                let mut shamir = ShamirPreprocessing::new(num_parties, threshold, 2, &net)
                    .unwrap()
                    .into();
                let mul = arithmetic::mul(x, y, &net, &mut shamir).unwrap();
                let mul = arithmetic::mul(mul, y, &net, &mut shamir).unwrap();
                tx.send(arithmetic::add(mul, x)).unwrap();
            });
        }
    ...
    fn shamir_mul_vec_bn_inner(num_parties: usize, threshold: usize) {
        ...
        for (net, tx, x, y) in izip!(nets, tx, x_shares, y_shares) {
            spawn_pool(move || {
                let mut shamir = ShamirPreprocessing::new(num_parties, threshold, x.len(), &net)
                    .unwrap()
                    .into();
                let mul = arithmetic::mul_vec(&x, &y, &net, &mut shamir).unwrap();
                tx.send(mul).unwrap();
            });
        }
    ...
    fn shamir_mul_vec_inner(num_parties: usize, threshold: usize) {
        ...
        for (net, tx, x, y) in izip!(nets, tx, x_shares, y_shares) {
            spawn_pool(move || {
                let mut shamir =
                    ShamirPreprocessing::new(num_parties, threshold, x.len() * 2, &net)
                        .unwrap()
                        .into();
                let mul = arithmetic::mul_vec(&x, &y, &net, &mut shamir).unwrap();
                let mul = arithmetic::mul_vec(&mul, &y, &net, &mut shamir).unwrap();
                tx.send(mul).unwrap();
            });
        }
    ...
    fn shamir_neg_inner(num_parties: usize, threshold: usize) {
        ...
        for (tx, x) in izip!(tx, x_shares) {
            spawn_pool(move || tx.send(arithmetic::neg(x)).unwrap());
        }
    ...
    fn shamir_inv_inner(num_parties: usize, threshold: usize) {
        ...
        for (net, tx, x) in izip!(nets, tx, x_shares) {
            spawn_pool(move || {
                let mut shamir = ShamirPreprocessing::new(num_parties, threshold, 1, &net)
                    .unwrap()
                    .into();
                tx.send(arithmetic::inv(x, &net, &mut shamir).unwrap())
                    .unwrap();
            });
        }
    ...
    fn shamir_poseidon2_gadget_kat1_inner(num_parties: usize, threshold: usize) {
        ...
        for (net, tx, x) in izip!(nets, tx, input_shares) {
            spawn_pool(move || {
                ...
                .into();
                let output = poseidon
                    .shamir_permutation(x.as_slice().try_into().unwrap(), &net, &mut shamir)
                    .unwrap()
                    .to_vec();
    ...
    fn shamir_poseidon2_gadget_kat1_precomp_inner(num_parties: usize, threshold: usize) {
        ...
        for (net, tx, x) in izip!(nets, tx, input_shares) {
            spawn_pool(move || {
                ...
                .unwrap()
                .into();
                let mut precomp = poseidon.precompute_shamir(1, &net, &mut shamir).unwrap();
                let output = poseidon
        ...
        for (net, tx, x) in izip!(nets, tx, input_shares) {
            spawn_pool(move || {
                ...
                let mut precomp = poseidon
                    .precompute_shamir(NUM_POSEIDON, &net, &mut shamir)
                    .unwrap();
                let output = poseidon
    ...
    fn shamir_poseidon2_merkle_tree_inner(num_parties: usize, threshold: usize) {
        ...
        for (net, tx, x) in izip!(nets, tx, input_shares) {
            spawn_pool(move || {
                let mut shamir = ShamirPreprocessing::new(num_parties, threshold, 0, &net)
                    .unwrap()
                    .into();

                let poseidon = Poseidon2::<_, 4, 5>::default();
                let output1 = poseidon
                    .merkle_tree_sponge_shamir::<2, _>(x.clone(), &net, &mut shamir)
                    .unwrap();
                let output2 = poseidon
                    .merkle_tree_compression_shamir::<4, _>(x, &net, &mut shamir)
                    .unwrap();
...
mod curve_share {
    ...
    fn shamir_add_inner(num_parties: usize, threshold: usize) {
        ...
        for (tx, x, y) in izip!(tx, x_shares, y_shares) {
            spawn_pool(move || tx.send(pointshare::add(&x, &y)).unwrap());
        }
    ...
    fn shamir_sub_inner(num_parties: usize, threshold: usize) {
        ...
        for (tx, x, y) in izip!(tx, x_shares, y_shares) {
            spawn_pool(move || tx.send(pointshare::sub(&x, &y)).unwrap());
        }
