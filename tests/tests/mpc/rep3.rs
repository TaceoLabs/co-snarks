mod field_share {
    ...
    use mpc_core::Fork;
    use mpc_engine::{Network, TestNetwork};
    ...
    fn rep3_sub_shared_by_public() {
        ...
        for (tx, x, id) in izip!(
            ...
            spawn_pool(move || tx.send(arithmetic::sub_shared_by_public(x, y, id)).unwrap());
        }
        let result1 = rx1.recv().unwrap();
        let result2 = rx2.recv().unwrap();
        let result3 = rx3.recv().unwrap();
    ...
    fn rep3_sub_public_by_shared() {
        ...
        for (tx, y, id) in izip!(
            ...
            spawn_pool(move || tx.send(arithmetic::sub_public_by_shared(x, y, id)).unwrap());
        }
        let result1 = rx1.recv().unwrap();
        let result2 = rx2.recv().unwrap();
        let result3 = rx3.recv().unwrap();
    ...
    #[test]
    fn rep3_mul() {
        let nets = TestNetwork::party_networks(3);
        let mut rng = thread_rng();
        let x = ark_bn254::Fr::rand(&mut rng);
        let y = ark_bn254::Fr::rand(&mut rng);
        let x_shares = rep3::share_field_element(x, &mut rng);
                    ...
                    for i in -1..=1 {
                        ...
                        for (net, tx, x, y, public) in izip!(
                            ...
                            spawn_pool(move || {
                                    ...
                                    tx.send([shared_compare, rhs_const]).unwrap();
                            });
                        }
    ...
    fn rep3_gc() {
        ...
        let [net1, net2, net3] = nets.try_into().unwrap();
    ...
    fn rep3_radix_sort() {
        ...
        let nets0 = TestNetwork::party_networks(3);
        let nets1 = TestNetwork::party_networks(3);
        let mut rng = thread_rng();
        let x = (0..VEC_SIZE)
            .map(|_| ark_bn254::Fr::rand(&mut rng))
            .collect_vec();
        ...
        for (net0, net1, tx, x_) in izip!(nets0, nets1, [tx1, tx2, tx3], x_shares.into_iter()) {
    ...
    fn rep3_poseidon2_gadget_kat1() {
        ...
        for (net, tx, x) in izip!(nets, [tx1, tx2, tx3], input_shares.into_iter()) {
            spawn_pool(move || {
                let mut state = Rep3State::new(&net).unwrap();
