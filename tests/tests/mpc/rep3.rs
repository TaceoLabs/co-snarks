mod field_share {
    ...
    fn slice_and_get_sparse_table_with_rotation_values<const BASE: u64>(
        ...
        assert_eq!(
            result.iter().map(|&x| ark_bn254::Fr::from(x)).collect_vec(),
            should_result
                .iter()
                .map(|&x| ark_bn254::Fr::from(x))
                .collect_vec()
