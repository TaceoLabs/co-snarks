mod field_share {
    ...
    #[expect(dead_code)]
    fn open_many<F: PrimeField>(
        net: &mut IoContext<tests::rep3_network::PartyTestNetwork>,
        a: Vec<crate::rep3::field_share::rep3::Rep3PrimeFieldShare<F>>,
    ) -> std::io::Result<Vec<F>> {
        let bs = a.iter().map(|x| x.b).collect_vec();
        net.network.send_next(bs)?;
        let mut cs = net.network.recv_prev::<Vec<F>>()?;
    ...
    fn slice_and_get_aes_sbox_values<const BASE: u64>(slice_sizes: Vec<u64>) {
        ...
        for (net, tx, x, y) in izip!(
            ...
            thread::spawn(move || {
                let mut rep3 = IoContext::init(net).unwrap();
                let mut rep3_2 = rep3.fork().unwrap();
                ...
                for key in rs.iter() {
                    let sbox_value = mpc_core::protocols::rep3_ring::lut::Rep3LookupTable::get_from_lut_internal_no_conversion::<u8, _>(
                        *key,
                        &sbox_lut,
                        &mut rep3,
                        &mut rep3_2,
                    ...
                    let and2a =
                        mpc_core::protocols::rep3::conversion::b2a(&and, &mut rep3).unwrap();
                    let and2a = arithmetic::mul_public(and2a, ark_bn254::Fr::from(0x1b));

                    let and2a =
                        mpc_core::protocols::rep3::conversion::a2b_selector(and2a, &mut rep3)
                            .unwrap();
                    ...
                    let bin_share: Vec<_> = mpc_core::protocols::rep3::conversion::bit_inject_many(
                        &a_bits_split,
                        &mut rep3,
                    )
                    .unwrap()
