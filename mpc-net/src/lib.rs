...
use bytes::Bytes;
use config::{Address, NetworkConfig};
        ...
        for (&id, conn) in self.connections.iter() {
            ...
            self.rt.spawn(async move {
                while let Some(frame) = send_rx.recv().await {
                    if write.send(Bytes::from(frame)).await.is_err() {
            ...
            self.rt.spawn(async move {
                while let Some(Ok(frame)) = read.next().await {
                    if recv_tx.send(frame.to_vec()).await.is_err() {
                        tracing::warn!("failed to send to recv receiver");
                    }
                }
            });
...
impl TcpNetwork {
    /// Create a new [TcpNetwork] for party with `id` and `bind_addr`.
    /// The `addrs` slice must be in the same order for all parties and must also contain the addr of this party.
    pub fn new(id: usize, bind_addr: impl ToSocketAddrs, addrs: &[Address]) -> eyre::Result<Self> {
        let [net] = Self::networks::<1>(id, bind_addr, addrs)?;
        Ok(net)
    }

    /// Create `N` new [TcpNetwork]s for party with `id` and `bind_addr`.
    /// The `addrs` slice must be in the same order for all parties and must also contain the addr of this party.
    pub fn networks<const N: usize>(
        id: usize,
            ...
            for (other_id, addr) in addrs.iter().enumerate() {
                match id.cmp(&other_id) {
                    Ordering::Less => {
                        ...
                        stream.set_write_timeout(Some(DEFAULT_CONNECTION_TIMEOUT))?;
...
impl Network for TcpNetwork {
    ...
    fn recv(&self, from: usize) -> eyre::Result<Vec<u8>> {
        let queue = self
            ...
            .context("while get stream in recv")?
            .lock();
        Ok(queue.recv_timeout(DEFAULT_CONNECTION_TIMEOUT)?)
    }
}
...
impl TlsNetwork {
    /// Create a new [TlsNetwork] for party with `id` and `bind_addr`.
    /// The `addrs` slice must be in the same order for all parties and must also contain the addr of this party.
    pub fn new(
        id: usize,
        bind_addr: impl ToSocketAddrs,
        ...
        let [net] = Self::networks::<1>(id, bind_addr, addrs, certs, key)?;
        Ok(net)
    }

    /// Create `N` new [TlsNetwork]s for party with `id` and `bind_addr`.
    /// The `addrs` slice must be in the same order for all parties and must also contain the addr of this party.
    pub fn networks<const N: usize>(
        id: usize,
        bind_addr: impl ToSocketAddrs,
