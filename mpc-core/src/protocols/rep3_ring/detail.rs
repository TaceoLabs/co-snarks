...
pub(super) fn low_depth_binary_add_many<T: IntRing2k, N: Network>(
    x1: &[Rep3RingShare<T>],
    x2: &[Rep3RingShare<T>],
    net: &N,
    state: &mut Rep3State,
...
pub(super) fn low_depth_binary_add<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
    ...
    Standard: Distribution<T>,
{
    ...
    kogge_stone_inner(&p, &g, net, state)
}

fn kogge_stone_inner<T: IntRing2k, N: Network>(
    p: &Rep3RingShare<T>,
    g: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
where
    Standard: Distribution<T>,
{
    let mut g = kogge_stone_loop(p.to_owned(), g.to_owned(), net, state)?;
    g <<= 1;
    g ^= p;
    Ok(g)
}
...
fn kogge_stone_inner_with_carry<T: IntRing2k, N: Network>(
    p: &Rep3RingShare<T>,
    g: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
) -> eyre::Result<(Rep3RingShare<T>, Rep3RingShare<Bit>)>
where
    Standard: Distribution<T>,
{
    let mut g = kogge_stone_loop(p.to_owned(), g.to_owned(), net, state)?;
...
fn kogge_stone_loop<T: IntRing2k, N: Network>(
    ...
    Standard: Distribution<T>,
{
    ...
    for i in 0..d {
        ...
        // TODO: Make and more communication efficient, ATM we send the full element for each level, even though they reduce in size
        // maybe just input the mask into AND?
        let (r1, r2) = and_twice(&p, &g_, &p_, net, state)?;
        p = r2;
        g ^= r1;
    }
    Ok(g)
...
fn and_twice<T: IntRing2k, N: Network>(
    a: &Rep3RingShare<T>,
    b1: &Rep3RingShare<T>,
    b2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
) -> eyre::Result<(Rep3RingShare<T>, Rep3RingShare<T>)>
where
    Standard: Distribution<T>,
{
    let (mut mask1, mask_b) = state.rngs.rand.random_elements::<RingElement<T>>();
...
fn low_depth_binary_sub_with_carry<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
// Calculates 2^k + x1 - x2
fn low_depth_binary_sub_by_const_with_carry<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &RingElement<T>,
    net: &N,
    state: &mut Rep3State,
...
// Calculates 2^k + x1 - x2
fn low_depth_binary_sub_from_const_with_carry<T: IntRing2k, N: Network>(
    x1: &RingElement<T>,
    x2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
pub(crate) fn unsigned_ge<T: IntRing2k, N: Network>(
    x: Rep3RingShare<T>,
    y: Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
    ...
    Standard: Distribution<T>,
{
    ...
    let (_, r) = low_depth_binary_sub_with_carry(&a_bits, &b_bits, net, state)?;
    Ok(r)
}
...
pub(crate) fn unsigned_ge_const_lhs<T: IntRing2k, N: Network>(
    x: RingElement<T>,
    y: Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
{
    let b_bits = conversion::a2b_selector(y, net, state)?;
    let (_, r) = low_depth_binary_sub_from_const_with_carry(&x, &b_bits, net, state)?;
    Ok(r)
}
...
pub(crate) fn unsigned_ge_const_rhs<T: IntRing2k, N: Network>(
    x: Rep3RingShare<T>,
    y: RingElement<T>,
    net: &N,
    state: &mut Rep3State,
...
{
    let a_bits = conversion::a2b_selector(x, net, state)?;
    let (_, r) = low_depth_binary_sub_by_const_with_carry(&a_bits, &y, net, state)?;
    Ok(r)
}
