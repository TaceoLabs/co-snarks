...
pub fn reshare_vec<F: PrimeField, N: Rep3Network>(
    ...
    if local_b.len() != local_a.len() {
        return Err(std::io::Error::new(
            ...
            "During execution of mul_vec: Invalid number of elements received",
        ));
    }
...
pub fn eq_many<F: PrimeField, N: Rep3Network>(
    ...
    if a.len() != b.len() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
           format!( "During execution of eq_many: Invalid number of elements received. Length of a : {} and length of b: {}",a.len(), b.len())
        ));
    }
...
pub fn eq_public_many<F: PrimeField, N: Rep3Network>(
    ...
    if shared.len() != public.len() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
           format!( "During execution of eq_public_many: Invalid number of elements received. Length of shared : {} and length of public: {}",shared.len(), public.len())
        ));
    }
...
pub fn eq_bit_public_many<F: PrimeField, N: Rep3Network>(
    ...
    if shared.len() != public.len() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
           format!( "During execution of eq_bit_public_many: Invalid number of elements received. Length of shared : {} and length of public: {}",shared.len(), public.len())
        ));
    }
    let public = public
        .iter()
...
pub fn eq_bit_many<F: PrimeField, N: Rep3Network>(
    ...
    if a.len() != b.len() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
           format!( "During execution of eq_bit_many: Invalid number of elements received. Length of a : {} and length of b: {}",a.len(), b.len())
        ));
    }
