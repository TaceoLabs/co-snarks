...
pub struct Binary<F: PrimeField, N: Rep3Network> {
    field: PhantomData<F>,
    network: PhantomData<N>,
}

impl<F: PrimeField, N: Rep3Network> Binary<F, N> {
    // this happens to compile time so we just do "as usize"
    ...
    pub fn xor_public(shared: &BinaryShare, public: &BigUint) -> BinaryShare {
        shared ^ public
    }
    ...
    pub async fn and(
        ...
        debug_assert!(a.a.bits() <= u64::try_from(Self::BITLEN).expect("usize fits into u64"));
        debug_assert!(b.a.bits() <= u64::try_from(Self::BITLEN).expect("usize fits into u64"));
        let (mut mask, mask_b) = io_context.rngs.rand.random_biguint(Self::BITLEN);
        mask ^= mask_b;
    ...
    pub async fn open(a: &BinaryShare, io_context: &mut IoContext<N>) -> IoResult<BigUint> {
