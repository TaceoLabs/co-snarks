...
use crate::sponge_hasher_mpc::{FieldHashRep3, FieldSpongeRep3, FieldSpongeRep3_};
...
pub trait TranscriptHasherRep3<F: PrimeField, U: NoirUltraHonkProver<C>, C: CurveGroup> {
...
> TranscriptHasherRep3<F, U, C> for FieldSpongeRep3<F, U, C, T, R, H>
{
    fn hash(buffer: Vec<F>) -> F {
        self::FieldSponge::<F, T, R, H>::hash_fixed_length::<1>(&buffer)[0]
    }
    fn hash_rep3<N: Network>(
        ...
        let res = self::FieldSpongeRep3_::<U, C, T, R, H>::hash_fixed_length::<1, N>(
            &buffer, net, mpc_state,
        )?[0];
        Ok(res)
    }
}
...
where
    ...
    C: HonkCurve<F>,
    H: TranscriptHasherRep3<F, T, C> + TranscriptHasher<F>,
{
    pub(crate) transcript_rep3: Option<TranscriptRep3_<F, T, C, H>>,
    pub(crate) transcript_plain: Option<Transcript<F, H>>,
}

pub struct TranscriptRep3_<F, T, C, H>
where
    F: PrimeField,
    T: NoirUltraHonkProver<C>,
    C: HonkCurve<F>,
    H: TranscriptHasherRep3<F, T, C>,
{
...
impl<F, T, C, H> Default for TranscriptRep3_<F, T, C, H>
where
    F: PrimeField,
    T: NoirUltraHonkProver<C>,
    C: HonkCurve<F>,
...
impl<F, T, C, H> TranscriptRep3_<F, T, C, H>
where
    F: PrimeField,
    T: NoirUltraHonkProver<C>,
    C: HonkCurve<F>,
    H: TranscriptHasherRep3<F, T, C>,
{
    ...
    pub fn send_fr_iter_to_verifier_shared<P: HonkCurve<F>>(
        &mut self,
        label: String,
        elements: &[T::ArithmeticShare],
    ) {
...
mod tests {
    ...
    use crate::transcript_mpc::TranscriptRep3_;
    use ark_ec::CurveGroup;
    ...
    fn rep3_transcript_poseidon_plaindriver() {
        ...
        let mut rep3_transcript = TranscriptRep3_::<
            TranscriptFieldType,
            ...
            >,
        >::new();
        let mut plain_transcript = Transcript::<TranscriptFieldType, Poseidon2Sponge>::new();
        ...
        rep3_transcript.send_fr_iter_to_verifier_shared::<ark_bn254::G1Projective>(
            "fr_iter".to_string(),
            &fr_vec,
        );
        let rep3_challenge = rep3_transcript
