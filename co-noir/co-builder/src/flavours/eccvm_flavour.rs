...
impl ECCVMFlavour {
    const _NUM_DERIVED_WITNESS_ENTITIES_NON_SHIFTED: usize = 1;
    ...
    //PRECOMPUTED ENTITIES (The other ones are already present in the Flavour)
    const LAGRANGE_SECOND: usize = 1; // column 1
...
impl ProverFlavour for ECCVMFlavour {
    ...
    const WITNESS_Z_PERM: usize = 85;
    const WITNESS_LOOKUP_INVERSES: usize = 86;
    ...
    type PolyG<'a, T: Default + 'a> = ECCVMPolyG<'a, T>;
}
pub struct ECCVMPolyG<'a, T: Default> {
    pub(crate) wires: &'a [T; ECCVMFlavour::SHIFTED_WITNESS_ENTITIES_SIZE],
}

impl<'a, T: Default> PolyGFlavour<'a, T> for ECCVMPolyG<'a, T> {
    ...
    fn from_slice(input: &'a [T]) -> Self {
        assert_eq!(
            input.len(),
            ECCVMFlavour::SHIFTED_WITNESS_ENTITIES_SIZE,
            "Input slice length does not match the expected size for ECCVM flavour."
        );
        Self {
            wires: input
                .try_into()
...
impl<T: Default + Debug> PrecomputedEntitiesFlavour<T> for ECCVMPrecomputedEntities<T> {
    ...
    fn lagrange_first_mut(&mut self) -> &mut T {
        &mut self.elements[ECCVMFlavour::LAGRANGE_FIRST]
    }
    fn lagrange_last_mut(&mut self) -> &mut T {
        &mut self.elements[ECCVMFlavour::LAGRANGE_LAST]
    }
    fn lagrange_first(&self) -> &T {
        &self.elements[ECCVMFlavour::LAGRANGE_FIRST]
    }
...
impl<T: Default + Debug> WitnessEntitiesFlavour<T> for ECCVMWitnessEntities<T> {
    ...
    fn to_be_shifted(&self) -> &[T] {
        &self.elements[ECCVMFlavour::TRANSCRIPT_MUL..=ECCVMFlavour::Z_PERM]
    }
    fn to_be_shifted_mut(&mut self) -> &mut [T] {
        &mut self.elements[ECCVMFlavour::TRANSCRIPT_MUL..=ECCVMFlavour::Z_PERM]
    }
    fn lookup_inverses_mut(&mut self) -> &mut T {
        &mut self.elements[ECCVMFlavour::LOOKUP_INVERSES]
    }
    fn lookup_inverses(&self) -> &T {
        &self.elements[ECCVMFlavour::LOOKUP_INVERSES]
    }
    fn z_perm(&self) -> &T {
...
impl<T: Default> ECCVMPrecomputedEntities<T> {
    pub fn lagrange_second_mut(&mut self) -> &mut T {
        &mut self.elements[ECCVMFlavour::LAGRANGE_SECOND]
    }
    pub fn lagrange_second(&self) -> &T {
        &self.elements[ECCVMFlavour::LAGRANGE_SECOND]
    }
}
...
impl<T: Default> ECCVMProverWitnessEntities<T> {
    ...
    pub fn into_shifted_without_z_perm(self) -> impl Iterator<Item = T> {
        self.elements
            .into_iter()
            .skip(ECCVMFlavour::TRANSCRIPT_MSM_COUNT_AT_TRANSITION_INVERSE + 1)
            .take(ECCVMFlavour::TRANSCRIPT_ACCUMULATOR_Y + 1 - ECCVMFlavour::TRANSCRIPT_MUL)
    }
    ...
    pub fn lookup_read_counts_0_mut(&mut self) -> &mut T {
        &mut self.elements[ECCVMFlavour::LOOKUP_READ_COUNTS_0]
    }
    pub fn lookup_read_counts_1_mut(&mut self) -> &mut T {
        &mut self.elements[ECCVMFlavour::LOOKUP_READ_COUNTS_1]
    }
}
