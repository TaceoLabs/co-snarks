...
impl<F: PrimeField> Polynomial<F> {
    ...
    pub fn interpolate_from_evals(
        ...
        let m = interpolation_points.len();
        let mut dest = Polynomial::new(vec![F::zero(); virtual_size]);
        debug_assert_eq!(m, evaluations.len());
        let mut numerator_polynomial = Vec::with_capacity(m + 1);
        let n = interpolation_points.len();

        let mut scratch_space = interpolation_points.to_vec();

        numerator_polynomial[n] = F::one();
        numerator_polynomial[n - 1] = -scratch_space.iter().copied().sum::<F>();
        ...
        for i in 0..n - 1 {
            ...
            numerator_polynomial[n - 2 - i] = temp * constant;
            constant *= -F::one();
        }
        let mut roots_and_denominators = Vec::with_capacity(2 * m);
        let mut tmp_src = Vec::with_capacity(m);
        for i in 0..m {
        ...
        let mut temp_dest = Vec::with_capacity(m);
        let mut idx_zero = 0;
