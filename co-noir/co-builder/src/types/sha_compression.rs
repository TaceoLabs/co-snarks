...
impl<F: PrimeField> SparseValue<F> {
    pub fn new<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        ...
        let sparse = if input.is_constant() {
            ...
            let _sparse_value = T::map_into_sparse_form(driver, 16, value);
            todo!()
            // FieldCT::new(input.get_context(), sparse_value)
        } else {
            FieldCT::default()
        };
...
// pub struct SparseChValue<F: PrimeField> {
//     pub normal: FieldCT<F>,
//     pub sparse: FieldCT<F>,
//     pub rot6: FieldCT<F>,
//     pub rot11: FieldCT<F>,
...
// pub struct SparseMajValue<F: PrimeField> {
//     pub normal: FieldCT<F>,
//     pub sparse: FieldCT<F>,
//     pub rot2: FieldCT<F>,
//     pub rot13: FieldCT<F>,
...
impl<F: PrimeField> SparseWitnessLimbs<F> {
    ...
    pub fn convert_witness<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        w: &FieldCT<F>,
...
impl<F: PrimeField> SHA256<F> {
    pub(crate) fn sha256_block<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        h_init: [FieldCT<F>; 8],
        input: [FieldCT<F>; 16],
        builder: &mut GenericUltraCircuitBuilder<P, T>,
        driver: &mut T,
    ...
    fn extend_witness<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        w_in: [FieldCT<F>; 16],
        builder: &mut GenericUltraCircuitBuilder<P, T>,
        driver: &mut T,
    ...
    fn choose<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        e: &mut SparseValue<F>,
        f: &SparseValue<F>,
        g: &SparseValue<F>,
        builder: &mut GenericUltraCircuitBuilder<P, T>,
    ...
    fn majority<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        a: &mut SparseValue<F>,
        b: &SparseValue<F>,
        c: &SparseValue<F>,
        builder: &mut GenericUltraCircuitBuilder<P, T>,
        ...
        if a.is_constant() && b.is_constant() {
            ...
            let overflow = if T::is_shared(&sum) {
                ...
                FieldCT::from_witness(
                    T::get_overflow_bit(driver, sum_val, 32, 64)?.into(),
                    builder,
                )
            } else {
    ...
    fn map_into_maj_sparse_form<P: Pairing<ScalarField = F>, T: NoirWitnessExtensionProtocol<F>>(
        e: FieldCT<F>,
        builder: &mut GenericUltraCircuitBuilder<P, T>,
        driver: &mut T,
...
fn get_choose_rotation_multipliers<F: PrimeField>() -> [F; 3] {
    let column_2_row_3_coefficients =
        [F::one(), F::from(28u64).pow([11]), F::from(28u64).pow([22])];

    // Scaling factors applied to a's sparse limbs, excluding the rotated limb
    let rot6_coefficients = [
        F::zero(),
        F::from(28u64).pow([11 - 6]),
        F::from(28u64).pow([22 - 6]),
    ];
    let rot11_coefficients = [
        F::from(28u64).pow([32 - 11]),
        F::zero(),
        F::from(28u64).pow([22 - 11]),
    ];
    let rot25_coefficients = [
        F::from(28u64).pow([32 - 25]),
        F::from(28u64).pow([32 - 25 + 11]),
        F::zero(),
    ];
    ...
    let column_2_row_1_multiplier = F::one() * target_rotation_coefficients[0]; // Why multiply by one?
...
fn get_majority_rotation_multipliers<F: PrimeField>() -> [F; 3] {
    let base_temp: u64 = 16;
    let base = F::from(base_temp);

    // Scaling factors applied to a's sparse limbs, excluding the rotated limb
    let rot2_coefficients = [F::zero(), base.pow([11 - 2]), base.pow([22 - 2])];
    let rot13_coefficients = [base.pow([32 - 13]), F::zero(), base.pow([22 - 13])];
    let rot22_coefficients = [base.pow([32 - 22]), base.pow([32 - 22 + 11]), F::zero()];
    ...
    let column_2_row_2_multiplier =
        target_rotation_coefficients[0] * (-base.pow([11])) + target_rotation_coefficients[1];
