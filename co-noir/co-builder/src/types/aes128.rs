...
use crate::{
    builder::GenericUltraCircuitBuilder, prelude::HonkCurve, types::field_ct::FieldCT,
    utils::Utils, TranscriptFieldType,
};
...
pub(crate) const AES128_SBOX: [u8; 256] = [
...
impl<F: PrimeField> AES128<F> {
    ...
    >(
        ...
        for byte in sparse_bytes[..16].iter() {
            let sparse_byte = byte.get_value(builder, driver);
            if !T::is_shared(&sparse_byte) {
                let mut sparse_byte: BigUint = T::get_public(&sparse_byte)
                ...
                todo!("Implement shared case");
            }
        }
        let result = FieldCT::<F>::from_witness(F::from(accumulator).into(), builder);
