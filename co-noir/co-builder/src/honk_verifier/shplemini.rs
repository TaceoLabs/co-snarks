...
impl ShpleminiVerifier {
    ...
    >(
        ...
        Self::remove_repeated_commitments(&mut commitments, &mut scalars, has_zk, builder, driver);
     ...
     * computed value here (no indicator-based freeze), matching bb exactly.
     *
     ...
     *    \f}
     *    The commitments \f$ [A_1]_1, \ldots, [A_{d-1}]_1 \f$ are multiplied by these scalars in the final
     *    `batch_mul` performed by KZG or IPA.
     *
    ...
    >(
        ...
        for j in 1..virtual_log_n {
            ...
            // the genuinely computed value here (no indicator-based freeze), matching bb exactly.
            scalars.push(
                scaling_factor_neg
    ...
    >(
        ...
        let offset = if has_zk == ZeroKnowledge::Yes { 2 } else { 1 };
    ...
    >(
        ...
        denominators[1] = one.divide(
            &shplonk_evaluation_challenge.sub(
                &subgroup_generator.multiply(gemini_evaluation_challenge, builder, driver)?,
                builder,
                driver,
            ),
            builder,
    ...
    >(
        ...
        for _ in challenge_poly_length..C::SUBGROUP_SIZE {
            challenge_polynomial_lagrange.push(zero.clone());
        }
    ...
    >(
        ...
        let temp = lagrange_last
            ...
            .sub(
                &vanishing_poly_eval.multiply(quotient_eval, builder, driver)?,
                builder,
                driver,
            );
        diff = diff.add(&temp, builder, driver);
    ...
    >(
        ...
        let mut denominators = Vec::with_capacity(C::SUBGROUP_SIZE);
        ...
        // at r
        for _ in 0..C::SUBGROUP_SIZE {
            denominators.push(
                running_power
                    .multiply(r, builder, driver)?
                    .sub(&one, builder, driver),
            ); // r * g^{-i} - 1
        ...
        let mut summed = zero.clone();
        for (coeff, denom_inv) in coeffs.iter().zip(denominators.iter()) {
            let term = coeff.multiply(denom_inv, builder, driver)?;
            summed = summed.add(&term, builder, driver);
        }

        let mut result = [
            summed,
            denominators[0].clone(),
            denominators[C::SUBGROUP_SIZE - 1].clone(),
        ];

        for entry in result.iter_mut() {
            *entry = entry.multiply(&numerator, builder, driver)?;
        }
