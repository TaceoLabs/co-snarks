...
use co_builder::{mega_builder::MegaCircuitBuilder, prelude::NUM_WIRES, transcript::{TranscriptCT, TranscriptHasherCT}, types::{field_ct::FieldCT, goblin_types::{GoblinElement, GoblinField}}};
...
impl MergeRecursiveVerifier {
    ...
    pub fn verify_proof<N: Network,
        ...
        >(&self, proof: Vec<FieldCT<P::ScalarField>>, builder: &mut MegaCircuitBuilder<P, T, D>, driver: &mut T, 
    ...
    for claim in opening_claims.iter().skip(1) {
        ...
        let tmp = alpha_pow.multiply(&claim.opening_pair.1, builder, driver).unwrap();
        batched_eval = batched_eval.add(&tmp, builder, driver);
        alpha_pow = alpha_pow.multiply(&alpha, builder, driver).unwrap();
    }

    let batched_commitment = Self::batch_mul(&commitments, &scalars, builder, driver, net, state);
    let opening_claim = OpeningClaim {
        commitment: batched_commitment,
        opening_pair: (kappa, batched_eval),
    };

    MergeRecursiveVerifier::reduce_verify(opening_claim, &mut transcript, builder, driver, net, state)

}

fn batch_mul<N: Network,
    P: HonkCurve<TranscriptFieldType, ScalarField = TranscriptFieldType>,
    T: NoirWitnessExtensionProtocol<TranscriptFieldType>,
    D: NoirUltraHonkProver<P, 
    ArithmeticShare = T::ArithmeticShare,
...
>(
    ...
    for i in 0..points.len() {
        ...
        let scalar_is_constant_equal_one = scalar.is_constant() && scalar.get_value(builder, driver) == P::ScalarField::ONE.into();
        let (x, y, is_point_at_infinity) = point.get_value(builder, driver);
        // TODO CESAR: Handle Unwrap
        let point_share = driver.field_shares_to_native_pointshare(x, y, is_point_at_infinity).unwrap();
        ...
        if !scalar_is_constant_equal_one {
            ...
            let beta = FieldCT::from_witness(P::ScalarField::get_root_of_unity(3).unwrap().into(), builder);
            scalar.assert_equal(&z_1.sub(&z_2.multiply(&beta, builder, driver).unwrap(), builder, driver), builder, driver);
        }
    }
    ...
    let op2_is_infinity = x_lo.add_two(&x_hi, &y_lo, builder, driver).add(&y_hi, builder, driver).is_zero(builder, driver).expect("is_zero should not fail");
    ...
    fn reduce_verify<N: Network,
        ...
        let quotient_commitment = transcript.receive_point_from_prover("KZG:W".to_owned(), builder, driver)?;
            ...
            let commitments = vec![
                ...
                GoblinElement::one(builder, driver),
            ];
            let scalars = vec![
                one.clone(),
