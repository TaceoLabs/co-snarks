...
>(
    ...
    let mut l_i_vec = Vec::with_capacity(log_poly_length as usize);
    let mut r_i_vec = Vec::with_capacity(log_poly_length as usize);

    // Step 6.
    // Perform IPA reduction rounds
    for _ in 0..log_poly_length {
        round_size /= 2;
        ...
        l_i = T::point_add(
            &l_i,
            &T::msm_public_points(&[aux_generator.into()], &[inner_prod_l]),
        );
        ...
        r_i = T::point_add(
            &r_i,
            &T::msm_public_points(&[aux_generator.into()], &[inner_prod_r]),
        );
        l_i_vec.push(l_i);
        r_i_vec.push(r_i);
    }
    ...
    let open_l_i = T::open_point_many(&l_i_vec, net, state)?;
    let open_r_i = T::open_point_many(&r_i_vec, net, state)?;

    for i in 0..log_poly_length {
        let index = (CONST_ECCVM_LOG_N - i as usize - 1).to_string();
        transcript
            .send_point_to_verifier::<P>(format!("IPA:L_{index}"), open_l_i[i as usize].into());
        transcript
            .send_point_to_verifier::<P>(format!("IPA:R_{index}"), open_r_i[i as usize].into());
        // Step 6.d
