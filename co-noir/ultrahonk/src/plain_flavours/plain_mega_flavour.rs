...
impl PlainProverFlavour for MegaFlavour {
    ...
    fn extend_and_batch_univariates_2<F: PrimeField, const SIZE: usize>(
        ...
        acc.r_arith
            .extend_and_batch_univariates_2(result, &[first_term, running_challenge[0].clone()]);
        acc.r_perm
            .extend_and_batch_univariates_2(result, &running_challenge[1..3]);
        acc.r_lookup
            .extend_and_batch_univariates_2(result, &running_challenge[3..5]);
        acc.r_delta
            .extend_and_batch_univariates_2(result, &running_challenge[5..9]);
        acc.r_elliptic
            .extend_and_batch_univariates_2(result, &running_challenge[9..11]);
        acc.r_aux
            .extend_and_batch_univariates_2(result, &running_challenge[11..17]);
        acc.r_ecc_op_queue
            .extend_and_batch_univariates_2(result, &running_challenge[17..25]);
        acc.r_databus
            .extend_and_batch_univariates_2(result, &running_challenge[25..31]);
        acc.r_pos_ext
            .extend_and_batch_univariates_2(result, &running_challenge[31..35]);
        acc.r_pos_int
            .extend_and_batch_univariates_2(result, &running_challenge[35..]);
    }
