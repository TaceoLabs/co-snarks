...
use crate::{
    ...
    decider::{
        ...
        sumcheck::{
            sumcheck_round_prover::SumcheckRoundOutput,
            sumcheck_round_verifier::SumcheckVerifierRound,
        },
    },
    ...
    DeciderVerifier<P, H>
{
    pub(crate) fn sumcheck_verify<const SIZE: usize>(
        ...
        for (round_idx, &padding_value) in padding_indicator_array.iter().enumerate() {
            ...
            let evaluations =
                transcript.receive_fr_array_from_prover::<P, SIZE>(round_univariate_label)?;
