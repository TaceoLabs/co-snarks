...
use crate::{prelude::Univariate, transcript::TranscriptFieldType};
...
use co_builder::{
    ...
    prelude::{HonkCurve, derive_generators},
};
use num_bigint::BigUint;

//TODO FLORIN: Move this somewhere else
pub(crate) fn offset_generator_scaled<C: CurveGroup>() -> C::Affine {
...
impl EccTranscriptRelation {
    ...
    pub(crate) fn accumulate<P: HonkCurve<TranscriptFieldType>, const SIZE: usize>(
        ...
        //     domain_bytes.push(i as u8);
        // }
        //     let offset_generator_base = derive_generators::<P::CycleGroup>(&domain_bytes, 1, 0)[0]; // we need CycleGroup here because all this happens in Grumpkin, thus offset_generator is a BN254 Curve point and therefore oxu and oyu are BN254 BaseField elements = Grumpkin ScalarField elements
