...
#[expect(dead_code)] // See commit_to_witness_polynomial for explanation why rng and has_zk are not used
...
impl<P: HonkCurve<TranscriptFieldType>, H: TranscriptHasher<TranscriptFieldType>> Oink<P, H> {
    ...
    fn execute_sorted_list_accumulator_round(
        ...
        let mut memory_w_4 = std::mem::take(&mut self.memory.w_4);
        self.commit_to_witness_polynomial(&mut memory_w_4, "W_4", &proving_key.crs, transcript)?;
        self.memory.w_4 = memory_w_4;
        Ok(())
    }
    ...
    fn execute_log_derivative_inverse_round(
        ...
        // TACEO TODO: BB does "sparse" commitment here, I don't know if that is necessary (performance wise)
        let mut lookup_inverses = std::mem::take(&mut self.memory.lookup_inverses);
        self.commit_to_witness_polynomial(
            &mut lookup_inverses,
            "LOOKUP_INVERSES",
            &proving_key.crs,
        ...
        self.memory.lookup_inverses = lookup_inverses;
        // Round is done since ultra_honk is no goblin flavor
        Ok(())
    }
    ...
    fn execute_grand_product_computation_round(
        ...
        let mut z_perm = std::mem::take(&mut self.memory.z_perm);
        self.commit_to_witness_polynomial(&mut z_perm, "Z_PERM", &proving_key.crs, transcript)?;
        self.memory.z_perm = z_perm;
        Ok(())
    }
