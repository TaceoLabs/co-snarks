...
use mpc_core::protocols::rep3::conversion::{a2b_selector, b2a};
...
impl<F: PrimeField, N: Rep3Network> NoirWitnessExtensionProtocol<F> for Rep3AcvmSolver<F, N> {
    ...
    fn slice_and_get_aes_sbox_values_from_key(
        ...
        for key in res0.iter() {
            if Self::is_shared(key) {
                let sbox_value = Rep3LookupTable::get_from_lut_internal_no_conversion::<u8, _>(
                    Self::get_shared(key).expect("Already checked it is shared"),
                    &sbox_lut,
                    &mut self.io_context0,
                    &mut self.io_context1,
                )?;
                ...
                let and2a = b2a(&and, &mut self.io_context0)?;
                let and2a = arithmetic::mul_public(and2a, F::from(0x1b));
                let and2a = a2b_selector(and2a, &mut self.io_context0)?;
                ...
                panic!("This case should really not happen");
            };
        }
