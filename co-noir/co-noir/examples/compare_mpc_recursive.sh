# This is afaik getting deprecated, but let's leave it here for now in case we need it
...
#   bash -c "$BARRETENBERG_BINARY $prove_command -b test_vectors/${name}/target/${name}.json -w test_vectors/${name}/target/${name}.gz --init_kzg_accumulator -o test_vectors/${name}/ $PIPE"
...
#   bash -c "$BARRETENBERG_BINARY $write_command -b test_vectors/${name}/target/${name}.json --init_kzg_accumulator -o test_vectors/${name}/ $PIPE"
...
#   echo "proving and verifying in co-noir and poseidon transcript"
