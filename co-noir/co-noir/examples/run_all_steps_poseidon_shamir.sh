...
cargo run --release --bin co-noir -- build-proving-key --witness test_vectors/poseidon/shamir_poseidon.gz.0.shared --circuit test_vectors/poseidon/poseidon.json --crs test_vectors/bn254_g1.dat --protocol SHAMIR --config configs/party1.toml --out proving_key.0.shared &
cargo run --release --bin co-noir -- build-proving-key --witness test_vectors/poseidon/shamir_poseidon.gz.1.shared --circuit test_vectors/poseidon/poseidon.json --crs test_vectors/bn254_g1.dat --protocol SHAMIR --config configs/party2.toml --out proving_key.1.shared &
cargo run --release --bin co-noir -- build-proving-key --witness test_vectors/poseidon/shamir_poseidon.gz.2.shared --circuit test_vectors/poseidon/poseidon.json --crs test_vectors/bn254_g1.dat --protocol SHAMIR --config configs/party3.toml --out proving_key.2.shared
# run proving in MPC
cargo run --release --bin co-noir -- generate-proof --proving-key proving_key.0.shared --protocol SHAMIR --hasher KECCAK --config configs/party1.toml --out proof.0.proof --public-input public_input.json &
cargo run --release --bin co-noir -- generate-proof --proving-key proving_key.1.shared --protocol SHAMIR --hasher KECCAK --config configs/party2.toml --out proof.1.proof &
cargo run --release --bin co-noir -- generate-proof --proving-key proving_key.2.shared --protocol SHAMIR --hasher KECCAK --config configs/party3.toml --out proof.2.proof
wait $(jobs -p)
