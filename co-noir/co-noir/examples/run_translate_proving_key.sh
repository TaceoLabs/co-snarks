...
# run proving in MPC
cargo run --release --bin co-noir -- generate-proof --proving-key shamir_proving_key.0.shared --protocol SHAMIR --hasher keccak --crs ../../co-noir-common/src/crs/bn254_g1.dat --config configs/party1.toml --out proof.0.proof --vk test_vectors/poseidon/verification_key --public-input public_input &
cargo run --release --bin co-noir -- generate-proof --proving-key shamir_proving_key.1.shared --protocol SHAMIR --hasher keccak --crs ../../co-noir-common/src/crs/bn254_g1.dat --config configs/party2.toml --out proof.1.proof --vk test_vectors/poseidon/verification_key &
cargo run --release --bin co-noir -- generate-proof --proving-key shamir_proving_key.2.shared --protocol SHAMIR --hasher keccak --crs ../../co-noir-common/src/crs/bn254_g1.dat --config configs/party3.toml --out proof.2.proof --vk test_vectors/poseidon/verification_key
wait $(jobs -p)
# verify proof
