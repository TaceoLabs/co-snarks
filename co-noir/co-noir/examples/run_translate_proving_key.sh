...
wait $(jobs -p)
# run proving in MPC
cargo run --release --bin co-noir -- generate-proof --proving-key shamir_proving_key.0.shared --protocol SHAMIR --hasher KECCAK --config configs/party1.toml --out proof.0.proof --public-input public_input.json &
cargo run --release --bin co-noir -- generate-proof --proving-key shamir_proving_key.1.shared --protocol SHAMIR --hasher KECCAK --config configs/party2.toml --out proof.1.proof &
cargo run --release --bin co-noir -- generate-proof --proving-key shamir_proving_key.2.shared --protocol SHAMIR --hasher KECCAK --config configs/party3.toml --out proof.2.proof
wait $(jobs -p)
