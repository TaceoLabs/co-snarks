...
run_proof_verification() {
 ...
 echo "comparing" $name "with bb version:" $BARRETENBERG_VERSION " and $algorithm transcript"
    ...
    bb $prove_command -b test_vectors/${name}/target/${name}.json -w test_vectors/${name}/target/${name}.gz -o test_vectors/${name}/${proof_file}
    ...
    bb $write_command -b test_vectors/${name}/target/${name}.json -o test_vectors/${name}/${vk_file}

    bb $verify_command -p test_vectors/${name}/proof -k test_vectors/${name}/vk
    if [[ $? -ne 0 ]]; then
    exit_code=1
    echo "::error::$name verifying with bb, our proof and our key failed"
  fi

    bb $verify_command -p test_vectors/${name}/proof -k test_vectors/${name}/${vk_file}
    if [[ $? -ne 0 ]]; then
    exit_code=1
    echo "::error::$name verifying with bb, our proof and their key failed"
  fi

    bb $verify_command -p test_vectors/${name}/${proof_file} -k test_vectors/${name}/vk
    if [[ $? -ne 0 ]]; then
    exit_code=1
    echo "::error::$name verifying with bb, their proof and our key failed"
    fi
  bb $verify_command -p test_vectors/${name}/${proof_file} -k test_vectors/${name}/${vk_file}
    if [[ $? -ne 0 ]]; then
    exit_code=1
