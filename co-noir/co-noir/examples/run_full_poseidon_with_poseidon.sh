...
cargo run --release --bin co-noir -- create-vk --circuit test_vectors/poseidon/poseidon.json --crs ../../co-noir-common/src/crs/bn254_g1.dat --hasher poseidon2 --vk test_vectors/poseidon/verification_key
# run proving in MPC
cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/poseidon/poseidon.gz.0.shared --circuit test_vectors/poseidon/poseidon.json --crs ../../co-noir-common/src/crs/bn254_g1.dat --protocol REP3 --hasher poseidon2 --config configs/party1.toml --out proof.0.proof --vk test_vectors/poseidon/verification_key --public-input public_input &
cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/poseidon/poseidon.gz.1.shared --circuit test_vectors/poseidon/poseidon.json --crs ../../co-noir-common/src/crs/bn254_g1.dat --protocol REP3 --hasher poseidon2 --config configs/party2.toml --out proof.1.proof --vk test_vectors/poseidon/verification_key &
...
wait $(jobs -p)
# verify proof
cargo run --release --bin co-noir -- verify --proof proof.0.proof --public-input public_input --vk test_vectors/poseidon/verification_key --hasher poseidon2 --crs ../../co-noir-common/src/crs/bn254_g2.dat
