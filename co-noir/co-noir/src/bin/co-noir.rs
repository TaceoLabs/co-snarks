...
impl_config!(CircuitInfoCli, CircuitConfig);
...
fn main() -> color_eyre::Result<ExitCode> {
    ...
    match args.command {
        ...
        Commands::CircuitInfo(cli) => {
            let config = CircuitConfig::parse(cli).context("while parsing config")?;
            ...
            tracing::info!("circuit_size = {circuit_size}");
            tracing::info!("abi = {abi}");
            tracing::info!("public_inputs = {public_inputs}");
            Ok(ExitCode::SUCCESS)
        }
    }
}
