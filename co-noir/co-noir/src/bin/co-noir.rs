...
fn run_translate_witness(config: TranslateWitnessConfig) -> color_eyre::Result<ExitCode> {
    ...
    let start = Instant::now();
    let (shamir_witness_shares, _) =
        co_noir::translated_witness::<Bn254, _, _>(witness_share, net)?;
    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
...
fn run_build_proving_key(config: BuildProvingKeyConfig) -> color_eyre::Result<ExitCode> {
    ...
    match protocol {
        MPCProtocol::REP3 => {
            ...
            let start = Instant::now();
            let (proving_key, _) = co_noir::generate_proving_key_rep3::<Bn254, _>(
                net,
                &constraint_system,
                witness_share,
                prover_crs.into(),
        ...
        MPCProtocol::REP3 => {
            ...
            match hasher {
                TranscriptHash::POSEIDON => {
                    ...
                    let start = Instant::now();
                    let (proof, _) =
                        Rep3CoUltraHonk::<_, _, Poseidon2Sponge>::prove(net, proving_key)?;
                    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
                ...
                TranscriptHash::KECCAK => {
                    ...
                    tracing::info!("Party {}: starting proof generation..", id);
                    let start = Instant::now();
                    let (proof, _) = Rep3CoUltraHonk::<_, _, Keccak256>::prove(net, proving_key)?;
                    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
                    tracing::info!("Party {}: Proof generation took {} ms", id, duration_ms);
                    (proof, public_input)
                }
        ...
        MPCProtocol::SHAMIR => {
            ...
            match hasher {
                TranscriptHash::POSEIDON => {
                    let start = Instant::now();
                    let (proof, _) =
                        ShamirCoUltraHonk::<_, _, Poseidon2Sponge>::prove(net, t, proving_key)?;
                    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
                    tracing::info!("Party {}: Proof generation took {} ms", id, duration_ms);
                ...
                TranscriptHash::KECCAK => {
                    let start = Instant::now();
                    let (proof, _) =
                        ShamirCoUltraHonk::<_, _, Keccak256>::prove(net, t, proving_key)?;
                    let duration_ms = start.elapsed().as_micros() as f64 / 1000.;
                    tracing::info!("Party {}: Proof generation took {} ms", id, duration_ms);
...
fn run_build_and_generate_proof(
    ...
    let (proof, public_input) = match protocol {
        MPCProtocol::REP3 => {
            ...
            let start = Instant::now();
            let (proving_key, net) = co_noir::generate_proving_key_rep3::<Bn254, _>(
                net,
                &constraint_system,
                witness_share,
                prover_crs.into(),
        ...
        MPCProtocol::SHAMIR => {
            ...
            tracing::info!("Party {}: starting to generate proving key..", id);
            let start = Instant::now();
            let (proving_key, net) = co_noir::generate_proving_key_shamir::<Bn254, _>(
                net,
                t,
                &constraint_system,
                witness_share,
