...
use crate::eccvm::ecc_op_queue::NUM_WNAF_DIGITS_PER_SCALAR;
use crate::eccvm::ecc_op_queue::POINT_TABLE_SIZE;
...
use co_builder::{
    ...
    prelude::{Polynomials, derive_generators},
};
...
impl<P: HonkCurve<TranscriptFieldType>> TranslationData<P> {
    pub(crate) fn new<H: TranscriptHasher<TranscriptFieldType>, R: rand::Rng + rand::CryptoRng>(
        ...
        let mut translation_data = Self::default();
    ...
    >(
        ...
        let mut small_ipa_prover = SmallSubgroupIPAProver::<P> {
            ...
            domain: GeneralEvaluationDomain::<P::ScalarField>::new(
                SmallSubgroupIPAProver::<P>::SUBGROUP_SIZE,
            )
...
pub(crate) fn batch_normalize<C: CurveGroup>(elements: &[C::Affine]) -> Vec<C::Affine> {
    let projective_elements: Vec<C> = elements.iter().map(|e| e.into_group()).collect();
    C::normalize_batch(&projective_elements)
}
pub(crate) fn batch_normalize_inplace<C: CurveGroup>(elements: &mut [C::Affine]) {
    let mut projective_elements: Vec<C> = elements.iter().map(|e| e.into_group()).collect();
    C::normalize_batch(&projective_elements);
    for (affine, projective) in elements.iter_mut().zip(projective_elements.iter()) {
...
impl<C: CurveGroup> VMState<C> {
    fn new() -> Self {
        Self {
            ...
            msm_accumulator: offset_generator::<C>(),
    ...
    fn process_msm_transition(
        ...
        if state.is_accumulator_empty {
            updated_state.accumulator =
                (updated_state.msm_accumulator - offset_generator::<C>()).into();
        } else {
            let r = state.accumulator;
            updated_state.accumulator =
                (r + updated_state.msm_accumulator - offset_generator::<C>()).into();
        }
        updated_state.is_accumulator_empty = updated_state.accumulator.is_zero();

        let msm_output = updated_state.msm_accumulator - offset_generator::<C>();
...
fn finalize_transcript<C: CurveGroup>(
    transcript_state: &mut [TranscriptRow<C::BaseField>],
    updated_state: &VMState<C>,
) where
    <C as CurveGroup>::BaseField: PrimeField,
{
    let final_row = transcript_state
        .last_mut()
        .expect("Transcript state should not be empty");
...
fn compute_rows<C: HonkCurve<TranscriptFieldType>>(
    ...
    let mut inverse_trace_x = Vec::with_capacity(num_vm_entries);
    let mut inverse_trace_y = Vec::with_capacity(num_vm_entries);
    let mut transcript_msm_x_inverse_trace = Vec::with_capacity(num_vm_entries);
    let mut add_lambda_denominator = Vec::with_capacity(num_vm_entries);
    let mut add_lambda_numerator = Vec::with_capacity(num_vm_entries);
    let mut msm_count_at_transition_inverse_trace = Vec::with_capacity(num_vm_entries);

    let mut msm_accumulator_trace = Vec::with_capacity(num_vm_entries);
    let mut accumulator_trace = Vec::with_capacity(num_vm_entries);
    let mut intermediate_accumulator_trace: Vec<C::Affine> = Vec::with_capacity(num_vm_entries);
    ...
    transcript_state[0] = TranscriptRow::<C::BaseField>::default();
    ...
    for i in 0..num_vm_entries {
        let mut row = &mut transcript_state[i + 1];
        ...
        if entry.op_code.reset {
            ...
            updated_state.msm_accumulator = offset_generator::<C>();
        }
        ...
        if msm_transition {
            VMState::<C>::process_msm_transition(row, &mut updated_state, &state);
        } else {
        ...
        if is_mul && next_not_msm {
            state.msm_accumulator = offset_generator::<C>();
        }
    }
    // compute affine coordinates of the accumulated points
    accumulator_trace = batch_normalize::<C>(&accumulator_trace);
    msm_accumulator_trace = batch_normalize::<C>(&msm_accumulator_trace);
    intermediate_accumulator_trace = batch_normalize::<C>(&intermediate_accumulator_trace);
    ...
    finalize_transcript(&mut transcript_state, &updated_state);
...
fn construct_from_builder<C: HonkCurve<TranscriptFieldType>>(
    ...
    let result = MSMRow::compute_rows_msms(&msms, number_of_muls, op_queue.get_num_msm_rows());
    ...
    for (i, row) in point_table_rows.iter().enumerate() {
        ...
        polys.witness.precompute_dx_mut()[i] = row
            .precompute_double
            .x()
            .expect("Double x-coordinate should not be zero");
        polys.witness.precompute_dy_mut()[i] = row
            .precompute_double
            .y()
            .expect("Double y-coordinate should not be zero");
    ...
    todo!("this->set_shifted() TODO FLORIN: Do we need to do this?")
}
