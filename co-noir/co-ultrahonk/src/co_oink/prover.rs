...
> CoOink<'a, T, P, H, N, L>
{
    ...
    fn compute_logderivative_inverses(
        ...
        if L::FLAVOUR == Flavour::Ultra {
            ...
            // NOTE regarding leaking: As these entries consist of nonzero values with high probability (compare compute_read and compute_write functions, there we have a challenge as an additive term), we do not leak zeroes here.
            ...
            .enumerate()
            {
                ...
                if !is_read_0_ {
                    is_read_0[i] = T::promote_to_trivial_share(id, P::ScalarField::zero());
                    to_compare_0.push(proving_key.polynomials.witness.calldata_read_counts()[i]);
                    bus_idx_0_indices.push(i);
                }
                if !is_read_1_ {
                    is_read_1[i] = T::promote_to_trivial_share(id, P::ScalarField::zero());
                    to_compare_1.push(
                        proving_key
                            .polynomials
                            .witness
                ...
                if !is_read_2_ {
                    is_read_2[i] = T::promote_to_trivial_share(id, P::ScalarField::zero());
            ...
            for (cmp, idx) in izip!(is_zero[..size_0].iter(), bus_idx_0_indices.iter()) {
                idx_0_compared[*idx] =
                    T::sub(T::promote_to_trivial_share(id, P::ScalarField::one()), *cmp);
            }
            for (cmp, idx) in izip!(
            ...
            self.memory.calldata_inverses = Polynomial::new(mul[0].to_vec());
            self.memory.secondary_calldata_inverses = Polynomial::new(mul[1].to_vec());
            self.memory.return_data_inverses = Polynomial::new(mul[2].to_vec());
            self.memory.lookup_inverses = Polynomial::new(mul[3].to_vec());
        }
        Ok(())
    }
