    ...
    for EccSetRelation
{
    ...
    fn accumulate<N: Network, const SIZE: usize>(
        ...
        let mut z_perm = input.witness.z_perm().to_owned();
        T::add_assign_public_many(&mut z_perm, lagrange_first, id);
        let z_perm_shift = input.shifted_witness.z_perm_shift();

        let mut lhs5 = Vec::with_capacity(3);
        ...
        let mut z_perm_shift_scaled = z_perm.to_owned();
        ...
        tmp.iter_mut()
            ...
            .for_each(|(a, b)| *a = T::mul_with_public(*b, *a));
