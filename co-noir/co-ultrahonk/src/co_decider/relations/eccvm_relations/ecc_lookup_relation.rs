...
impl EccLookupRelation {
    ...
    >(
        ...
        match write_index {
            0 => {
                let mut positive_slice_value = precompute_round.to_owned();
                T::scale_many_in_place(&mut positive_slice_value, P::ScalarField::from(-1));
    ...
    for EccLookupRelation
{
    ...
    fn accumulate<N: Network, const SIZE: usize>(
        ...
        for (i, term) in lookup_terms
            ...
            .enumerate()
        {
            *term = Self::compute_write_term::<T, P, SIZE>(input, relation_parameters, i, id)
                .try_into()
                .unwrap_or_else(|_| panic!("vec must contain exactly {SIZE} elements"));
        }
        ...
        rhs.extend(row_has_write.to_owned());
        ...
        for i in 0..NUM_TOTAL_TERMS - 1 {
            // TODO FLORIN: Is there a better way?
            denominator_accumulator[NUM_TOTAL_TERMS - 1 - i] = T::mul_many(
        ...
        let mut lhs = Vec::with_capacity(Self::READ_TERMS + Self::WRITE_TERMS);
        let mut rhs = Vec::with_capacity(Self::READ_TERMS + Self::WRITE_TERMS);
