    ...
    for EccWnafRelation
{
    ...
    fn accumulate<N: Network, const SIZE: usize>(
        ...
        scaled_transition
            ...
            .for_each(|(a, b)| {
                T::mul_assign_with_public(a, *b);
            });

        let scaled_transition_is_zero = scaled_transition
            .iter()
            .zip_eq(scaling_factors)
            .map(|(a, b)| T::add_with_public(*b, T::mul_with_public(minus_one, *a), id))
            .collect_vec();
        ...
        // acc 20 // TODO FLORIN: do with half shares (here and prob everywhere else)
        fold_accumulator!(
