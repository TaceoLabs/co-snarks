    ...
    for EllipticRelation
{
    ...
    fn accumulate(
        ...
        let x_diff = x_2.sub(x_1);
        let y1_plus_y3 = y_1.add(y_3);
        let y_diff = y_2.mul_public(q_sign).sub(y_1);
        let x1_mul_3 = x_1.add(x_1).add(x_1);
        ...
        let y1_sqr = &mul1[0];
        let y1_sqr_mul_4 = y1_sqr.add(y1_sqr);
        let y1_sqr_mul_4 = y1_sqr_mul_4.add(&y1_sqr_mul_4);
        let x1_sqr_mul_3 = &mul1[6];

        let lhs = SharedUnivariate::univariates_to_vec(&[
            x_3.add(x_2).add(x_1),
            y1_sqr.sub_scalar(curve_b, party_id),
            x_3.add(x_1).add(x_1),
        ...
        let y2_sqr = &mul1[1];
        let y1y2 = mul1[2].mul_public(q_sign);
        let x_add_identity = mul2[0].sub(y2_sqr).sub(y1_sqr).add(&y1y2).add(&y1y2);
        ...
        let tmp_1 = x_add_identity.mul_public(&q_elliptic_not_double_scaling);

        ///////////////////////////////////////////////////////////////////////
        // Contribution (2) point addition, x-coordinate check
        // q_elliptic * (q_sign * y1 + y3)(x2 - x1) + (x3 - x1)(y2 - q_sign * y1) = 0
        let y_add_identity = &mul1[4].add(&mul1[5]);
        let tmp_2 = y_add_identity.mul_public(&q_elliptic_not_double_scaling);
        ...
        let x1_pow_4_mul_9 = x_pow_4_mul_3.add(x_pow_4_mul_3).add(x_pow_4_mul_3);
        let x_double_identity = mul2[2].sub(&x1_pow_4_mul_9);

        let tmp = x_double_identity.mul_public(&q_elliptic_q_double_scaling);
        let tmp_1 = tmp_1.add(&tmp);

        ///////////////////////////////////////////////////////////////////////
        // Contribution (4) point doubling, y-coordinate check
        // (y1 + y1) (2y1) - (3 * x1 * x1)(x1 - x3) = 0
        let y_double_identity = mul2[3].sub(&mul2[4]);
        let tmp = y_double_identity.mul_public(&q_elliptic_q_double_scaling);
        let tmp_2 = tmp_2.add(&tmp);
        ...
        for i in 0..univariate_accumulator.r1.evaluations.len() {
            univariate_accumulator.r1.evaluations[i] = T::add(
    ...
    fn accumulate_batch(
        ...
        rhs.extend(x_1);
        let mul1 = driver.mul_many(&lhs, &rhs)?;
        // we need the different contributions
        ...
        for (idx, b) in tmp_1.iter().enumerate() {
        ...
        let mut acc = [T::ArithmeticShare::default(); MAX_PARTIAL_RELATION_LENGTH];
        for (idx, b) in tmp_2.iter().enumerate() {
            let a = &mut acc[idx % MAX_PARTIAL_RELATION_LENGTH];
            T::add_assign(a, *b);
        }
        univariate_accumulator
