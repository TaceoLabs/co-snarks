    ...
    Relation<T, P, L> for DataBusLookupRelation
{
    ...
    fn can_skip(entity: &super::ProverUnivariates<T, P, L>) -> bool {
        ...
        entity.precomputed.q_busread().is_zero()
            && entity.witness.calldata_read_counts().is_zero()
            && entity.witness.secondary_calldata_read_counts().is_zero()
            && entity.witness.return_data_read_counts().is_zero()
    }
