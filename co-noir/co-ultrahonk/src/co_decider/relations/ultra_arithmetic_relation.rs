...
macro_rules! rayon_join {
    ...
    ($a: expr, $b: expr, $c: expr) => {{
        let (a, (b,c)) = rayon::join(|| $a, || rayon_join!($b, $c))
        (a,b,c)
    }};

    ($a: expr, $b: expr, $c: expr, $d: expr) => {{
        let ((a,b),(c,d)) = rayon_join!(rayon_join!($a, $b), rayon_join!($c, $d));
        (a,b,c,d)
    }};
}
...
#[derive(Clone, Debug)]
pub(crate) struct UltraArithmeticRelationAccNotReshared<T: NoirUltraHonkProver<P>, P: Pairing> {
...
impl<T: NoirUltraHonkProver<P>, P: Pairing> Default
    for UltraArithmeticRelationAccNotReshared<T, P>
{
    fn default() -> Self {
        Self {
            r0: Default::default(),
...
impl UltraArithmeticRelation {
        ...
        P: HonkCurve<TranscriptFieldType>,
    {
        ...
        // FRANCO TODO make this min_len configurable??
...
{
    type Acc = UltraArithmeticRelationAccNotReshared<T, P>;
