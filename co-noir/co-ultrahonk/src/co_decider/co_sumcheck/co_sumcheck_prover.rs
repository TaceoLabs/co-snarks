...
> CoDecider<'_, T, P, H, N, L>
{
    ...
    pub fn sumcheck_prove_zk<const VIRTUAL_LOG_N: usize>(
        ...
        if L::IS_GRUMPKIN_FLAVOUR {
            round_univariate_evaluations[multivariate_d as usize - 1][2] = T::eval_poly(
    ...
    fn commit_to_round_univariate(
        ...
        let eval_at_0 = T::eval_poly(
            round_univariate.evaluations_as_ref(),
            P::ScalarField::zero(),
        );
        let eval_at_1 = T::eval_poly(round_univariate.evaluations_as_ref(), P::ScalarField::one());
