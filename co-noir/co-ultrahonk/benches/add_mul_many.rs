...
type FieldType = Fp<MontBackend<FrConfig, 4>, 4>;
...
fn add_mul_many(c: &mut Criterion) {
    ...
    for num_elements in todo_list {
        ...
        let mut group = c.benchmark_group(format!("assign_many_{}_elems_{}_threads", num_elements, num_threads));
        ...
        let mut group = c.benchmark_group(format!("loop_{}_elems_{}_threads", num_elements, num_threads));
        group.throughput(Throughput::Elements(num_elements));
        group.bench_function("go", |b| {
            b.iter(|| {
                let mut acc = [zero; MAX_PARTIAL_RELATION_LENGTH];
        ...
        let mut group = c.benchmark_group(format!("loop_rayon_{}_elems_{}_threads", num_elements, num_threads));
        group.throughput(Throughput::Elements(num_elements));
        group.bench_function("go", |b| {
            b.iter(|| {
                w_l.par_iter()
