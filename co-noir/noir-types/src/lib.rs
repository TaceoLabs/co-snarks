...
pub type Input = BTreeMap<String, serde_json::Value>;
    ...
    for param in partial_abi.parameters.iter() {
        ...
        loop {
            let next = orig_params
                .next()
                .ok_or(eyre::eyre!("Corrupted Witness: Too few witnesses"))?;
...
fn json_type_from_value(value: serde_json::Value) -> JsonTypes {
    match value {
        serde_json::Value::String(s) => JsonTypes::String(s),
        serde_json::Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                JsonTypes::Integer(i)
            } else if let Some(u) = n.as_u64() {
                JsonTypes::Integer(u as i64)
            } else {
                // If the number is too large, we will not be able to parse it as an i64
                // and will need to handle it as a string.
                JsonTypes::String(n.to_string())
            }
        }
        serde_json::Value::Bool(b) => JsonTypes::Bool(b),
        serde_json::Value::Array(arr) => JsonTypes::Array(
            arr.into_iter()
                .map(json_type_from_value)
                .collect::<Vec<_>>(),
        ),
        serde_json::Value::Object(obj) => JsonTypes::Table(
            obj.into_iter()
                .map(|(k, v)| (k, json_type_from_value(v)))
                .collect(),
        ),
...
fn parse_json(
    ...
    let json_inputs = input
        .into_iter()
        .map(|(name, value)| (name, json_type_from_value(value)))
        .collect::<BTreeMap<String, JsonTypes>>();
