...
impl<F: PrimeField> Rep3BrilligType<F> {
    ...
    pub fn into_arithmetic_share<N: Network>(
        shared: Shared<F>,
        net: &N,
        state: &mut Rep3State,
        ...
        match shared {
            ...
            Shared::Ring128(share) => {
                Ok(rep3_ring::casts::ring_to_field_selector(share, net, state)?)
            }
            Shared::Ring64(share) => {
                Ok(rep3_ring::casts::ring_to_field_selector(share, net, state)?)
            }
        ...
        + AsPrimitive<u128>,
{
    match integer_bit_size {
        IntegerBitSize::U1 => Ok(Rep3BrilligType::Shared(Shared::Ring1(
            rep3_ring::casts::ring_cast_selector::<_, Bit, _>(share, net, state)?,
        ))),
        IntegerBitSize::U8 => Ok(Rep3BrilligType::Shared(Shared::Ring8(
            rep3_ring::casts::ring_cast_selector::<_, u8, _>(share, net, state)?,
        ))),
        IntegerBitSize::U16 => Ok(Rep3BrilligType::Shared(Shared::Ring16(
            rep3_ring::casts::ring_cast_selector::<_, u16, _>(share, net, state)?,
        ))),
        IntegerBitSize::U32 => Ok(Rep3BrilligType::Shared(Shared::Ring32(
            rep3_ring::casts::ring_cast_selector::<_, u32, _>(share, net, state)?,
        ))),
        IntegerBitSize::U64 => Ok(Rep3BrilligType::Shared(Shared::Ring64(
            rep3_ring::casts::ring_cast_selector::<_, u64, _>(share, net, state)?,
        ))),
        IntegerBitSize::U128 => Ok(Rep3BrilligType::Shared(Shared::Ring128(
            rep3_ring::casts::ring_cast_selector::<_, u128, _>(share, net, state)?,
        ))),
    }
}
...
impl<F: PrimeField, N: Network> BrilligDriver<F> for Rep3BrilligDriver<'_, F, N> {
    ...
    fn fork(&mut self) -> eyre::Result<(Self, Self)> {
        ...
        // because the are run in sequence we can just clone the net, but if we run them in parallel in the future we must fork the networks
        let net0 = self.net;
        let net1 = self.net;
        ...
        match (val, bit_size) {
            ...
            (Rep3BrilligType::Shared(shared), BitSize::Integer(integer_bit_size)) => match shared {
                ...
                Shared::Ring128(rep3_ring_share) => {
                    cast_ring(rep3_ring_share, integer_bit_size, self.net, &mut self.state)
                }
        ...
        let result = match (lhs, rhs) {
            ...
            (Rep3BrilligType::Shared(s1), Rep3BrilligType::Shared(s2)) => match (s1, s2) {
                (Shared::Field(s1), Shared::Field(s2)) => Rep3BrilligType::shared_field(
                    rep3::arithmetic::mul(s1, s2, self.net, &mut self.state)?,
                ),
                (Shared::Ring128(s1), Shared::Ring128(s2)) => Rep3BrilligType::shared_u128(
                    rep3_ring::arithmetic::mul(s1, s2, self.net, &mut self.state)?,
                ),
    ...
    fn div(
        ...
        let result = match (lhs, rhs) {
            ...
            (Rep3BrilligType::Public(public), Rep3BrilligType::Shared(shared)) => {
                match (public, shared) {
                    (Public::Field(lhs), Shared::Field(rhs)) => {
                        Rep3BrilligType::shared_field(rep3::arithmetic::div_public_by_shared(
                            lhs,
                            rhs,
                            self.net,
                            &mut self.state,
            ...
            (Rep3BrilligType::Shared(s1), Rep3BrilligType::Shared(s2)) => match (s1, s2) {
                (Shared::Field(lhs), Shared::Field(rhs)) => Rep3BrilligType::shared_field(
                    rep3::arithmetic::div(lhs, rhs, self.net, &mut self.state)?,
                ),

                (Shared::Ring128(lhs), Shared::Ring128(rhs)) => {
                    let divided = rep3_ring::yao::ring_div(lhs, rhs, self.net, &mut self.state)?;
                    Rep3BrilligType::shared_u128(divided)
                }
        ...
        let result = match (lhs, rhs) {
            ...
            (Rep3BrilligType::Public(public), Rep3BrilligType::Shared(shared)) => {
                if let (Public::Field(public), Shared::Field(shared)) = (public, shared) {
                    let divided = rep3::yao::field_int_div_by_shared(
                        public,
                        shared,
                        self.net,
                        &mut self.state,
            ...
            (Rep3BrilligType::Shared(s1), Rep3BrilligType::Shared(s2)) => {
                if let (Shared::Field(s1), Shared::Field(s2)) = (s1, s2) {
                    let divided = rep3::yao::field_int_div(s1, s2, self.net, &mut self.state)?;
                    Rep3BrilligType::shared_field(divided)
                } else {
                    eyre::bail!("IntDiv only supported on fields")
                }
    ...
    fn eq(
        ...
        let result = match (lhs, rhs) {
            ...
            | (Rep3BrilligType::Shared(secret), Rep3BrilligType::Public(public)) => {
                match (secret, public) {
                    (Shared::Field(secret), Public::Field(public)) => {
                        let eq = rep3::arithmetic::eq_bit_public(
                            secret,
                            public,
                            self.net,
                            &mut self.state,
            ...
            (Rep3BrilligType::Shared(s1), Rep3BrilligType::Shared(s2)) => match (s1, s2) {
                ...
                (Shared::Ring128(s1), Shared::Ring128(s2)) => Rep3BrilligType::shared_u1(
                    rep3_ring::arithmetic::eq(s1, s2, self.net, &mut self.state)?,
                ),
    ...
    fn lt(
        ...
        let result = match (lhs, rhs) {
            ...
            (Rep3BrilligType::Shared(s1), Rep3BrilligType::Shared(s2)) => match (s1, s2) {
                ...
                (Shared::Ring128(s1), Shared::Ring128(s2)) => Rep3BrilligType::shared_u1(
                    rep3_ring::arithmetic::lt(s1, s2, self.net, &mut self.state)?,
                ),
                (Shared::Ring64(s1), Shared::Ring64(s2)) => Rep3BrilligType::shared_u1(
                    rep3_ring::arithmetic::lt(s1, s2, self.net, &mut self.state)?,
                ),
    ...
    fn le(
        ...
        let result = match (lhs, rhs) {
            ...
            (Rep3BrilligType::Shared(s1), Rep3BrilligType::Shared(s2)) => match (s1, s2) {
                ...
                (Shared::Ring128(s1), Shared::Ring128(s2)) => Rep3BrilligType::shared_u1(
                    rep3_ring::arithmetic::le(s1, s2, self.net, &mut self.state)?,
                ),
                (Shared::Ring64(s1), Shared::Ring64(s2)) => Rep3BrilligType::shared_u1(
                    rep3_ring::arithmetic::le(s1, s2, self.net, &mut self.state)?,
                ),
                ...
                if let (Shared::Field(val), Public::Int(radix, IntegerBitSize::U32)) = (val, radix)
                {
                    ...
                    if radix.is_power_of_two() {
                        ...
                        for i in (0..output_size).rev() {
                            ...
                            if bits {
                                let limb_bit = rep3_ring::binary::is_zero(
                                    &limb[0],
                                    self.net,
                                    &mut self.state,
                                )?;
            ...
            (Rep3BrilligType::Public(val), Rep3BrilligType::Shared(radix)) => {
                if let (Public::Field(val), Shared::Ring32(radix)) = (val, radix) {
                    ...
                    let mut limbs: Vec<Rep3BrilligType<_>> =
                        vec![Rep3BrilligType::default(); output_size];
                    let radix_as_field =
                        rep3_ring::yao::ring_to_field_many(&[radix], self.net, &mut self.state)?;
                    ...
                    let limb = rep3::arithmetic::sub_public_by_shared(
                        val,
                        rep3::arithmetic::mul(div, radix_as_field[0], self.net, &mut self.state)?,
                        self.id,
                    );
                    let limb = rep3_ring::yao::field_to_ring_many::<_, u8, _>(
                        &[limb],
                    ...
                    for i in (0..output_size).rev().skip(1) {
                        ...
                        if bits {
                            let limb_bit =
                                rep3_ring::binary::is_zero(&limb[0], self.net, &mut self.state)?;
                            limbs[i] = Rep3BrilligType::Shared(Shared::<F>::Ring1(limb_bit));
                        } else {
                            limbs[i] = Rep3BrilligType::Shared(Shared::<F>::Ring8(limb[0]));
                        }
