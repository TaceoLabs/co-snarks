...
macro_rules! to_shared_field_cts {
    ($x:expr, $builders:expr) => {
        share_field_element(field_from_hex_string($x.as_str()).unwrap(), &mut rand::thread_rng())
        .into_iter()
...
fn to_goblin_elements(
    ...
    let z = share_field_element(Fr::from(z), &mut rand::thread_rng()).into_iter().zip(builders.iter_mut())
        .map(|(share, builder)| BoolCT::from_witness_ct(WitnessCT::from_acvm_type(share.into(), builder), builder));
    izip!(x0, x1, y0, y1, z).map(|(x0, x1, y0, y1, z)| {
...
fn test_recursive_verifier_rep3() {
    ...
    for (proof, builder) in izip!(honk_proofs.iter_mut(), builders.iter_mut()) {
        *proof = honk_proof
            .iter()
            .map(|s| FieldCT::from_witness(field_from_hex_string::<Fr>(s.as_str()).unwrap().into(), builder))
            .collect::<Vec<_>>();
    }
    ...
    let relation_parameters = [relation_parameters.0, relation_parameters.1, relation_parameters.2].into_iter().map(
        |x| RelationParameters {
            eta_1: x[0].clone(),
            eta_2: x[1].clone(),
            eta_3: x[2].clone(),
            beta: x[3].clone(),
    ...
    let accumulators = izip!(
        ...
        precomputed_commitments,
        witness_commitments,
    ).map(|(circuit_size, log_circuit_size, num_public_inputs, pub_inputs_offset, target_sum, gate_challenges, alphas, relation_parameters, precomputed_commitments, witness_commitments)| 
    ...
    let expected_pairing_data = [pairing_point_1, pairing_point_2].into_iter().flat_map(|((x0, x1), (y0, y1), z)| {
    ...
    for (net_1, net_2, accumulator, builder, honk_proof) in izip!(nets_1, nets_2, accumulators, builders, honk_proofs) {
        ...
        threads.push(std::thread::spawn(move || {
            ...
            let result = [p1, p2].into_iter().flat_map(|point| [
                point.x.limbs[0].get_value(&mut builder, &mut driver),
                point.x.limbs[1].get_value(&mut builder, &mut driver),
                point.y.limbs[0].get_value(&mut builder, &mut driver),
                point.y.limbs[1].get_value(&mut builder, &mut driver),
